                ...
                data: (items) {
                  final filtered = items.where((item) {
                    final matchesCategory =
                        categoryId == null ||
                        item.recipe.categoryId == categoryId;
                    final matchesQuery =
                        query.trim().isEmpty ||
                        item.recipe.title.toLowerCase().contains(
                  ...
                  return ListView.separated(
                    ...
                    itemBuilder: (context, i) => RecipeCard(
                      item: filtered[i],
                      onTap: () =>
                          context.push('/recipes/${filtered[i].recipe.id}'),
                    ),
                  );
                },
                loading: () => const Center(child: CircularProgressIndicator()),
                error: (e, s) =>
                    EmptyState(message: 'Nie udało się wczytać przepisów: $e'),
              ),
            ),
          ],
        ),
