...
final recipesProvider = StreamProvider<List<RecipeWithCategory>>(
...
class RecipeRepository {
  ...
  Stream<List<RecipeWithCategory>> watchAll() {
    final query = _db.select(_db.recipes).join([
      innerJoin(
        _db.categories,
        _db.categories.id.equalsExp(_db.recipes.categoryId),
      ),
    ])..orderBy([OrderingTerm.desc(_db.recipes.updatedAt)]);
    return query.watch().map(
      (rows) => rows
          .map(
            (row) => RecipeWithCategory(
  ...
  Future<void> delete(int id) =>
      (_db.delete(_db.recipes)..where((r) => r.id.equals(id))).go();
}
