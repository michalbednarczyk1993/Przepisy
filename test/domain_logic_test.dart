...
void main() {
  test('category names are unique case-insensitively', () {
    final categories = [
      RecipeCategory(id: 1, name: 'Obiad', createdAt: DateTime(2026)),
    ];
  ...
  test('recipe filter matches category and title query', () {
    ...
    expect(
      recipeMatchesFilters(recipe, categoryId: 2, query: 'kurczak'),
      isTrue,
    );
    expect(
      recipeMatchesFilters(recipe, categoryId: 3, query: 'kurczak'),
      isFalse,
    );
    expect(
      recipeMatchesFilters(recipe, categoryId: 2, query: 'sernik'),
      isFalse,
    );
  });
}
